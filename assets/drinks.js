  ...
  try {
    ...
    for (let i = 0; i < data.drinks.length; i++) {
      ...
      if (counter < 6) {
        ...
        cardHeadDiv.setAttribute('class', 'card-header is-flex-direction-column is-align-content-center');
        let h1 = document.createElement('h1');
        h1.setAttribute('class', 'card-header-title is-size-4 is-justify-content-center');
    ...
    for (i = 1; i < 16; i++) {
      if (data.drinks[0][`strIngredient${i}`] == null) {
        ...
        if (data.drinks[0][`strMeasure${i}`] !== null) {
          ingredient.innerText = data.drinks[0][`strMeasure${i}`] + ': ' + data.drinks[0][`strIngredient${i}`];
